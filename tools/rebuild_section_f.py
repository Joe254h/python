...
OUT = '/home/user/python/output/Trader_Questionnaire_FULL_v3.docx'
...
SHORT = {
 ...
 'hours': lambda v: v.replace('–', ' to ').replace('-', ' to ').replace(':00', ''),
...
def short(key, value):
    try:
        ...
        raise SystemExit(f"no short form for {key}: {value!r}")
...
spec = [('Attributes', 'Seven: cost, location, opening hours, caregiver ratio and '
        ...
        ('Criterion', 'D-efficient (D-error 0.545), generated in R by coordinate exchange with zero priors and a level-balance penalty'),
